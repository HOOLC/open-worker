...
- `zork-client-core`：77 个单测和相关 API、客户端边界、设置副本提交、重连/投递等集成回归通过；`zork-client-types` 的 21 个单测通过。设置回归还覆盖仅推进存储游标不唤醒 UI，以及任务延后启动仍能发布已过期的在线状态且不重复唤醒。需要独立 Gateway 的 enrollment 用例按其配置忽略，本轮未执行该环境测试。
