...
手机页只显示二维码接入；其它设备页只显示 Station 加入命令。两种邀请在
切换时各自保留，切换本身不生成新邀请。手机在「连接设备」中选择
「扫一扫连接」，也可以粘贴邀请。桌面显示手机名称并请求确认；点击
「允许连接」后，手机自动保存获准的设备，加载 Leader 和会话。

无需额外云控制面。Station 保存邀请和授权，Synch 验证身份并传输，
已有 relay/discovery 配置用于跨网连接。负责成员管理的 Station 在首次
授权时需要在线；完成后不需要桌面客户端保持打开。

## 短票据与握手

默认在线票据固定 68 字符：`zj1_`（Station）或 `zc1_`（手机），后面是
...
- Station 的未完成邀请只保存在有界内存表中，15 分钟有效、一次使用；
  Station 重启使未完成邀请失效。用途在服务端绑定，改前缀不能改变权限。
- 在加密握手内返回设备身份、名称、有效期和后续网络设置；接收方验证
  返回信息与票据匹配，再沿用 Synch 身份证明和桌面确认。
- 手机只能成为 `clients`，不成为 Station 或 Worker。桌面确认绑定
  具体手机身份和 claim ID，确认前没有普通 Station API 权限。
...
旧 Station 没有手机邀请接口，需要更新 Station 和桌面端后才能出码。

## 验证（2026-09-07）

- 31 项 Rust 单元/回归测试通过（Mesh、core、共享格式、JNI）。
- `scripts/android/test_enrollment.py`：真实隔离 Station，验证确认前无授权、
  错误 claim 拒绝、邀请抢占拒绝、进程恢复、重复扫码、撤销、拒绝及输入校验。
- 既有 `scripts/test-mesh-enrollment.py` 也通过：三 Station 入网、回执恢复、
...
- `PhoneEnrollmentTest` 在 Android 16 模拟器和 OPPO PLP110 / Find N6
  ...
  成功读取 Station 并重连。测试使用独立目录和隔离 Station，不接入生产节点。
...
真实三 Station 和手机客户端授权回归通过；Find N6 已完成真实桌面二维码
