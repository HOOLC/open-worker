...
- GPUI 统计和 hover 共享 Conversation overview topic；普通聊天呈现与 Android 消息 wire 不订阅这个 topic。Gateway 任务计数从 snapshot 建立，游标单独推进不产生业务唤醒。
...
- Gateway：只有最近一条运行记录时仍从 snapshot 得到 100 次累计运行；重复/仅游标推进不触发同步版本或业务唤醒，新运行和重启后的计数仍正确。
...
| Gateway | 93 项通过，含并发首快照及任务计数/静默游标回归 |
...
首次 Gateway 回归发现游标记账触发额外 WORK 唤醒，修复后完整复测通过。统计页原拖拽 fixture 没有保留抓取偏移，已改为按实际分隔条位置往返，保留原宽度断言。并行新增代码的两处编译引用/错误转换问题做了最小适配。链接一度因磁盘不足失败；确认进程已退出后，只清理本任务的过期测试二进制，并成功重新构建。
...
cargo test --locked -p zork-gateway --bin zork-gateway
