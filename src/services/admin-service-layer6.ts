...
import type { DeployReleaseOptions } from "./deploy/release-deployment-service.js";
...
import { AdminServiceLayer5 } from "./admin-service-layer5.js";
export class AdminServiceLayer6 extends AdminServiceLayer5 {
  async deleteSession(options: { readonly sessionKey: string }): Promise<Record<string, unknown>> {
    return await this.privateRunTrackedOperation(
      ...
      async () => {
        ...
        const workerDelete = await this.privateDeleteWorkerSession(session.key);
        const removedArtifacts = await this.privateRemoveDeletedSessionArtifacts(session.key, jobs);
        await this.privateRefreshSessions();
  ...
  async deployRelease(options: { readonly target: "admin" | "worker"; readonly version: string; readonly allowActive: boolean }): Promise<Record<string, unknown>> {
    ...
    return await this.privateRunTrackedOperation(
      ...
      async () => {
        await this.privateAssertSafeToInterrupt(options.allowActive, "deploy");
