...
export class AdminServiceLayer1 extends AdminServiceBase {
  getAdminUiBootstrap(): {
  ...
  async getStatus(): Promise<Record<string, unknown>> {
    const snapshot = await this.privateReadSessionSnapshot();
    const runtime = await this.privateReadRuntimeStatus();
    const usage = this.privateReadUsageOverview();
    const channelLabels = await this.privateBuildChannelLabelLookup(snapshot.allSessions, snapshot.inboundBySession);
    const sessionSummaries = snapshot.allSessions.slice(0, 50).map((session: SlackSessionRecord) =>
      this.privateSummarizeSession(session, {
    ...
    const stateCounts = this.privateSummarizeStateCounts(snapshot);

    return {
      service: this.privateServiceInfo(),
      authFiles: {
        authJson: await this.privateFileInfo(path.join(this.options.config.codexHome, "auth.json")),
        credentialsJson: await this.privateFileInfo(path.join(this.options.config.codexHome, ".credentials.json")),
        configToml: await this.privateFileInfo(path.join(this.options.config.codexHome, "config.toml")),
      },
      ...
      realtime: this.privateRealtimeInfo(),
      usage,
      operations: this.privateListAdminOperations(10),
      auditEvents: this.privateListAdminAuditEvents({ limit: 10 }),
      state: {
        ...stateCounts,
        activeSessions: snapshot.activeSessions,
        openInbound: snapshot.openInbound.slice(0, 25).map((message: PersistedInboundMessage) => this.privateSummarizeInbound(message)),
        sessions: sessionSummaries,
        recentBrokerLogs: await this.privateReadRecentBrokerLogs(40),
      },
    };
  }
  ...
  async getOverview(): Promise<Record<string, unknown>> {
    ...
    return {
      ...
      operations: this.privateListAdminOperations(10),
      auditEvents: this.privateListAdminAuditEvents({ limit: 10 }),
      state: this.privateSummarizeCachedStateCounts(),
    };
  }
  ...
  async getRecentLogs(
    ...
    return {
      ok: true,
      logs: await this.privateReadRecentBrokerLogs(limit),
    };
  }
  ...
  async listSessionSummaries(): Promise<Record<string, unknown>> {
    ...
    return {
      ok: true,
      realtime: this.privateRealtimeInfo(),
      sessions: snapshot.allSessions.slice(0, 500).map((session: SlackSessionRecord) =>
        this.privateSummarizeSessionListItem(session, {
          inbound: snapshot.inboundBySession.get(session.key) ?? [],
          openInbound: snapshot.openInboundBySession.get(session.key) ?? [],
          jobs: snapshot.jobsBySession.get(session.key) ?? [],
          usage: snapshot.usageBySession.get(session.key),
