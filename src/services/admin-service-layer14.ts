...
import { AdminServiceLayer13 } from "./admin-service-layer13.js";
export class AdminServiceLayer14 extends AdminServiceLayer13 {
  privateGetAgentSessionUsageSummary(sessionKey: string): PersistedAgentSessionUsageSummary | undefined {
    const store = this.privateOperationStore();
    return store.getAgentSessionUsageSummary?.call(this.options.sessions, sessionKey);
  }

  privateListAgentTraceEventsPage(
    sessionKey: string,
    options: {
      readonly limit?: number | undefined;
      readonly beforeSequence?: number | undefined;
  ...
  privateListVisibleTimelineTraceEventsPage(
    sessionKey: string,
    options: {
    ...
    while (scanned < maxRawScan) {
      const page = this.privateListAgentTraceEventsPage(sessionKey, {
    ...
    const returnedTraceSequences = selected.map((event) => event.sequence).filter((sequence): sequence is number => typeof sequence === "number");
    const nextBeforeSequence = returnedTraceSequences.length ? Math.min(...returnedTraceSequences) : rawCursor;
    return {
  ...
  privateGetAgentSessionTraceSummary(sessionKey: string): PersistedAgentSessionTraceSummary | undefined {
    const store = this.privateOperationStore();
    return store.getAgentSessionTraceSummary?.call(this.options.sessions, sessionKey);
  }

  privateOperationStore(): AdminOperationStore {
    return this.options.sessions as unknown as AdminOperationStore;
  }

  async privateReadAccountSummary(): Promise<SerializedAccountStatus> {
    try {
      return serializeAccountSummary(await withAdminProbeTimeout(this.options.runtime.readAccountSummary(false), "account summary"));
    } catch (error) {
      return serializeAccountError(error);
    }
  }

  async privateReadAccountRateLimits(): Promise<SerializedRateLimitsStatus> {
    try {
      return serializeRateLimits(await withAdminProbeTimeout(this.options.runtime.readAccountRateLimits(), "account rate limits"));
    } catch (error) {
      return serializeRateLimitsError(error);
    }
  }

  async privateReadDeploymentStatus(): Promise<unknown> {
