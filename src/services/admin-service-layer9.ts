...
import { AdminServiceLayer8 } from "./admin-service-layer8.js";
export class AdminServiceLayer9 extends AdminServiceLayer8 {
  async privateReadRuntimeStatus(): Promise<RuntimeStatus> {
    await this.options.githubAuthorMappings.load();
    await this.options.githubPrIdentity?.load();
    const [account, rateLimits, deployment, authProfiles] = await Promise.all([this.privateReadAccountSummary(), this.privateReadAccountRateLimits(), this.privateReadDeploymentStatus(), this.privateReadAuthProfilesStatus()]);
    ...
    const slackIdentities = await this.privateReadGitHubAccountSlackIdentities({
      bindings: prBindings,
      sessions,
    });
  ...
  async privateReadGitHubAccountSlackIdentities(options: { readonly bindings: readonly GitHubPrBindingRecord[]; readonly sessions: readonly SlackSessionRecord[] }): Promise<ReadonlyMap<string, SlackUserIdentity>> {
    ...
    const userIds = [...collectKnownGitHubAccountSlackUserIds(options)].filter((userId) => looksLikeSlackUserId(userId)).slice(0, 100);
    if (!userIds.length) {
      return new Map();
    }
    ...
    try {
      entries = await withAdminProbeTimeout(
        Promise.all(
          userIds.map(async (userId): Promise<readonly [string, SlackUserIdentity] | null> => {
