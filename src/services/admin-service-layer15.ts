...
import { AdminServiceLayer14 } from "./admin-service-layer14.js";
export class AdminServiceLayer15 extends AdminServiceLayer14 {
  async privateReadAuthProfilesStatus(): Promise<unknown> {
    try {
  ...
  async privateAssertSafeToInterrupt(allowActive: boolean, action: string): Promise<void> {
    if (allowActive) {
      return;
    }

    const preflight = await this.privateReadOperationPreflight(action);
    if (preflight.requiresAllowActive) {
      throw new Error(`Refusing ${action} while admin work is in flight (activeCount=${preflight.activeCount}, openInboundCount=${preflight.openInboundCount}, runningBackgroundJobCount=${preflight.runningBackgroundJobCount}). Retry with allow_active=true if you really want to interrupt it.`);
    }
  }

  async privateRefreshSessions(): Promise<void> {
  ...
  async privateFileInfo(filePath: string): Promise<FileInfo> {
    try {
  ...
  async privateReadRecentBrokerLogs(limit: number): Promise<readonly unknown[]> {
    if (limit <= 0) {
      return [];
    }
  ...
  privateSummarizeInbound(message: PersistedInboundMessage): Record<string, unknown> {
  ...
  privateSummarizeJob(job: PersistedBackgroundJob): Record<string, unknown> {
    return {
      id: job.id,
