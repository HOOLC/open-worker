...
import { AdminServiceLayer3 } from "./admin-service-layer3.js";
export class AdminServiceLayer4 extends AdminServiceLayer3 {
  ...
  async completeAuthProfileDeviceCode(options: { readonly name?: string | undefined; readonly deviceAuthId: string; readonly userCode: string; readonly retryAfterSeconds?: number | undefined }): Promise<Record<string, unknown>> {
  ...
  async deleteAuthProfile(options: { readonly name: string }): Promise<Record<string, unknown>> {
    return await this.privateRunTrackedOperation(
      "auth_profile_delete",
      {
        name: options.name,
  ...
  async switchSessionAuthProfile(options: { readonly sessionKey: string; readonly name?: string | undefined; readonly mode?: "auto" | undefined }): Promise<Record<string, unknown>> {
    const selectionMode = options.mode === "auto" ? "auto" : "manual";
    const request: JsonLike =
      selectionMode === "auto"
        ? {
    ...
    return await this.privateRunTrackedOperation("session_auth_profile_switch", request, async () => {
      ...
      const profile = selectionMode === "auto" ? selectBestAuthProfile(status) : options.name ? findAuthProfile(status, options.name) : null;
      if (!profile) {
      ...
      await this.privateAppendSessionAuthProfileSwitchTrace(switched, profile.name, selectionMode);
      const workerResume = await this.privateResumeWorkerPendingSession(switched.key);
      return {
        ok: true,
        selectedMode: selectionMode,
        selectedProfileName: profile.name,
        session: this.privateSummarizeSessionByKey(switched.key),
        workerResume,
      };
    });
  }
