...
import { AdminServiceLayer2 } from "./admin-service-layer2.js";
export class AdminServiceLayer3 extends AdminServiceLayer2 {
  ...
  async getOperationPreflight(options: { readonly operation: string }): Promise<Record<string, unknown>> {
    return {
      ok: true,
      ...(await this.privateReadOperationPreflight(options.operation)),
    };
  }
  ...
  async listAdminOperations(): Promise<Record<string, unknown>> {
    return {
      ok: true,
      operations: this.privateListAdminOperations(50),
    };
  }

  async listAdminAuditEvents(options?: { readonly operationId?: string | undefined }): Promise<Record<string, unknown>> {
    return {
      ok: true,
      events: this.privateListAdminAuditEvents({
  ...
  async listRealtimeEvents(options: { readonly afterSequence: number; readonly limit?: number | undefined }): Promise<Record<string, unknown>> {
    ...
    return {
      ok: true,
      cursor: events.at(-1)?.sequence ?? this.privateLatestRealtimeSequence(),
      events: events.map((event) => this.privateSerializeRealtimeEvent(event)),
    };
  }

  getRealtimeCursor(): number {
    return this.privateLatestRealtimeSequence();
  }

  async addAuthProfile(options: { readonly name?: string | undefined; readonly authJsonContent: string }): Promise<Record<string, unknown>> {
