...
import type { RollbackReleaseOptions } from "./deploy/release-deployment-service.js";
...
import { AdminServiceLayer6 } from "./admin-service-layer6.js";
export class AdminServiceLayer7 extends AdminServiceLayer6 {
  async rollbackRelease(options: { readonly target: "admin" | "worker"; readonly version?: string | undefined; readonly allowActive: boolean }): Promise<Record<string, unknown>> {
    ...
    return await this.privateRunTrackedOperation(
      ...
      async () => {
        await this.privateAssertSafeToInterrupt(options.allowActive, "rollback");
  ...
  async upsertGitHubAuthorMapping(options: { readonly slackUserId: string; readonly githubAuthor: string }): Promise<Record<string, unknown>> {
    return await this.privateRunTrackedOperation(
  ...
  async deleteGitHubAuthorMapping(options: { readonly slackUserId: string }): Promise<Record<string, unknown>> {
    return await this.privateRunTrackedOperation(
  ...
  async setDefaultGitHubPrAccount(options: { readonly slackUserId: string }): Promise<Record<string, unknown>> {
