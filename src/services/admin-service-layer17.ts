...
import type {} from "./session-manager.js";
import type {} from "./auth-profile-service.js";
import type {} from "./github-author-mapping-service.js";
import type { GitHubPrBindingRecord, GitHubPrIdentityStatus } from "./github-pr-identity-service.js";
import type {} from "./runtime-control.js";
...
import { AdminServiceLayer16 } from "./admin-service-layer16.js";
export class AdminServiceLayer17 extends AdminServiceLayer16 {
  privateSummarizeInboundListMessage(message: PersistedInboundMessage): Record<string, unknown> {
  ...
  async privateAppendSessionAuthProfileSwitchTrace(session: SlackSessionRecord, profileName: string, selectionMode: "manual" | "auto"): Promise<void> {
  ...
  async privateResumeWorkerPendingSession(sessionKey: string): Promise<Record<string, unknown>> {
    const url = new URL(`/slack/sessions/${encodeURIComponent(sessionKey)}/resume-pending`, this.options.config.workerBaseUrl);
  ...
  async privateDeleteWorkerSession(sessionKey: string): Promise<Record<string, unknown>> {
    const url = new URL(`/slack/sessions/${encodeURIComponent(sessionKey)}`, this.options.config.workerBaseUrl);
    const response = await fetch(url, {
      method: "DELETE",
    });
    const payload = (await response.json().catch(() => ({}))) as Record<string, unknown>;
  ...
  async privateResetWorkerSession(sessionKey: string): Promise<Record<string, unknown>> {
    const url = new URL(`/slack/sessions/${encodeURIComponent(sessionKey)}/reset`, this.options.config.workerBaseUrl);
    const response = await fetch(url, {
      method: "POST",
    });
    const payload = (await response.json().catch(() => ({}))) as Record<string, unknown>;
  ...
  async privateCancelWorkerBackgroundJob(sessionKey: string, jobId: string): Promise<Record<string, unknown>> {
    const url = new URL(`/jobs/${encodeURIComponent(jobId)}/admin-cancel`, this.options.config.workerBaseUrl);
    const response = await fetch(url, {
      method: "POST",
      headers: {
