...
import { AdminServiceLayer10 } from "./admin-service-layer10.js";
export class AdminServiceLayer11 extends AdminServiceLayer10 {
  privateSerializeRealtimeEvent(event: PersistedAdminEvent): Record<string, unknown> {
    ...
    if (event.sessionKey && event.kind !== "trace.append") {
      serialized.session = this.privateSummarizeSessionByKey(event.sessionKey) ?? null;
    }
  ...
  privateSummarizeSessionByKey(sessionKey: string): Record<string, unknown> | null {
    ...
    const channelLabels = this.privateKnownChannelLabelLookup([session], new Map([[session.key, inbound]]));
    return this.privateSummarizeSession(session, {
      inbound,
      openInbound,
      jobs,
      usage: summarizeUsageBySessionMap(this.privateListAgentTurnUsage(1000)).get(session.key),
      channelLabels,
    });
  }

  async privateBuildChannelLabelLookup(sessions: readonly SlackSessionRecord[], inboundBySession: ReadonlyMap<string, readonly PersistedInboundMessage[]>): Promise<ReadonlyMap<string, string>> {
    const labels = this.privateKnownChannelLabelLookup(sessions, inboundBySession);
    const missingChannelIds = uniqueChannelIds(sessions).filter((channelId) => !labels.has(channelId) && looksLikeSlackConversationId(channelId));
    ...
    await Promise.all(
      missingChannelIds.map(async (channelId) => {
        const label = await this.privateResolveSlackChannelLabel(channelId);
        if (label) {
  ...
  privateKnownChannelLabelLookup(sessions: readonly SlackSessionRecord[], inboundBySession: ReadonlyMap<string, readonly PersistedInboundMessage[]>): Map<string, string> {
    const labels = buildChannelLabelLookup(sessions, inboundBySession);
    for (const session of sessions) {
      const cached = this.privateChannelLabelCache.get(session.channelId);
      if (cached) {
  ...
  async privateResolveSlackChannelLabel(channelId: string): Promise<string | undefined> {
    if (this.privateChannelLabelCache.has(channelId)) {
      return this.privateChannelLabelCache.get(channelId) ?? undefined;
    }
    ...
    try {
      ...
      this.privateChannelLabelCache.set(channelId, label ?? null);
      return label;
    } catch (error) {
      ...
      this.privateChannelLabelCache.set(channelId, null);
      return undefined;
    }
  }
}
