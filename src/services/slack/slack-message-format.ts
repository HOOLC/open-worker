...
function formatBackgroundJobEventForAgent(message: SlackInputMessage): string {
  ...
  return [
    ...
    "If this async event only confirms a completion you already told the Slack thread about, record a silent final state through `zork-call chat post-state` instead of posting another completion message.",
    "If you need to record that you are still intentionally waiting without speaking in Slack, use `zork-call chat post-state --kind wait` instead of posting another wait message.",
...
function formatUnexpectedTurnStopForAgent(message: SlackInputMessage): string {
  ...
  return [
    ...
    "If the thread already has a clear completion update from you and you only need to settle broker state, record a silent final state through `zork-call chat post-state` instead of sending another completion message.",
    ...
    "If your visible Slack reply already explains the blocker, record a silent block state through `zork-call chat post-state` instead of sending a second state-only Slack message.",
    "If you are intentionally waiting on a broker-managed async job that is already running and will wake this session later, you may either send a Slack update with kind=wait or record a silent wait state through `zork-call chat post-state`.",
