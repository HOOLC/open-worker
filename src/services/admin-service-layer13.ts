...
import { AdminServiceLayer12 } from "./admin-service-layer12.js";
export class AdminServiceLayer13 extends AdminServiceLayer12 {
  async privateRunTrackedOperation<T extends Record<string, unknown>>(kind: AdminOperationKind, request: JsonLike, operationBody: () => Promise<T>): Promise<T & { readonly operation: PersistedAdminOperation }> {
    ...
    await this.privatePersistAdminOperation(operation);
    await this.privateAppendAdminAuditEvent({
      id: randomUUID(),
  ...
  async privatePersistAdminOperation(operation: PersistedAdminOperation): Promise<void> {
  ...
  async privateAppendAdminAuditEvent(event: PersistedAdminAuditEvent): Promise<void> {
    const store = this.privateOperationStore();
  ...
  privateListAdminOperations(limit: number): readonly PersistedAdminOperation[] {
    const store = this.privateOperationStore();
    return store.listAdminOperations?.call(this.options.sessions, limit) ?? [];
  }

  privateListAdminAuditEvents(options: { readonly operationId?: string | undefined; readonly limit: number }): readonly PersistedAdminAuditEvent[] {
    const store = this.privateOperationStore();
    return store.listAdminAuditEvents?.call(this.options.sessions, options) ?? [];
  }

  privateListAgentTurnUsage(limit: number): readonly PersistedAgentTurnUsage[] {
    const store = this.privateOperationStore();
    return store.listAgentTurnUsage?.call(this.options.sessions, limit) ?? [];
  }

  privateListAgentSessionUsageSummaries(): readonly PersistedAgentSessionUsageSummary[] {
    const store = this.privateOperationStore();
