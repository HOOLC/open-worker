...
export interface AdminServiceBase {
  [key: string]: any;
}

export class AdminServiceBase {
  readonly privateChannelLabelCache = new Map<string, string | null>();
...
function channelHumanLabelForSession(session: SlackSessionRecord, inbound: readonly PersistedInboundMessage[]): string | undefined {
  ...
  const channelName = inbound.map((message) => readStringField(message.slackMessage, "channel_name")).find((value) => value);
  if (channelName) {
