...
import { AdminServiceLayer1 } from "./admin-service-layer1.js";
export class AdminServiceLayer2 extends AdminServiceLayer1 {
  async getSessionTimeline(
    ...
    const tracePage = this.privateListVisibleTimelineTraceEventsPage(session.key, {
      limit: pageLimit,
      beforeSequence: options.beforeSequence,
    });
    const timelinePage = selectTimelinePageEvents({
      agentEvents: tracePage.events.map((event: PersistedAgentTraceEvent) => agentTraceEventToTimelineEvent(event)),
      limit: pageLimit,
    ...
    return {
      ok: true,
      session: this.privateSummarizeSession(session, {
        inbound,
        openInbound,
        jobs,
        usage: sessionUsageSummaryFromPersisted(this.privateGetAgentSessionUsageSummary(session.key)),
        channelLabels: await this.privateBuildChannelLabelLookup([session], new Map([[session.key, inbound]])),
      }),
      trace: traceSummaryFromPersisted(this.privateGetAgentSessionTraceSummary(session.key), tracePage.events),
      page: {
        limit: pageLimit,
