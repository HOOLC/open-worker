...
import { AdminServiceLayer9 } from "./admin-service-layer9.js";
export class AdminServiceLayer10 extends AdminServiceLayer9 {
  async privateReadSessionSnapshot(): Promise<SessionSnapshot> {
    ...
    const backgroundJobs = this.options.sessions.listBackgroundJobs().sort((left, right) => String(right.updatedAt ?? "").localeCompare(String(left.updatedAt ?? "")));
    ...
    const usageBySession = summarizeUsageBySessionMapFromPersisted(this.privateListAgentSessionUsageSummaries());
    const allSessions = sessions.sort((left, right) =>
  ...
  privateSummarizeStateCounts(snapshot: SessionSnapshot): Record<string, unknown> {
  ...
  privateSummarizeCachedStateCounts(): Record<string, unknown> {
    const sessions = this.options.sessions.listSessions();
  ...
  privateReadUsageOverview(): Record<string, unknown> {
    const records = this.privateListAgentTurnUsage(1000);
    ...
    return {
      totals,
      windows,
      recentTurns: records.slice().sort(compareUsageRecordsDescending).slice(0, 25).map(summarizeUsageRecord),
      bySession: summarizeUsageBySession(records, 25),
    };
  }

  privateRealtimeInfo(): Record<string, unknown> {
    return {
      cursor: this.privateLatestRealtimeSequence(),
    };
  }

  privateLatestRealtimeSequence(): number {
    const getLatest = (
      this.options.sessions as {
