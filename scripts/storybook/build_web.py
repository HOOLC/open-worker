...
def main():
 p=argparse.ArgumentParser();p.add_argument('--output',type=Path,default=ROOT/'apps/zork-design/components/web');args=p.parse_args();output=args.output;output.mkdir(parents=True,exist_ok=True)
 ...
 subprocess.run([str(tool),'--target','web','--no-typescript','--out-dir',str(output/'pkg'),str(target_root/'wasm32-unknown-unknown/debug/zork_gui_web.wasm')],check=True)
 ...
 for name in ['index.html','main.js']:shutil.copy2(Path(__file__).parent/'web'/name,output/name)
 (output/'build.json').write_text(json.dumps({'package':'zork-gui-web','component_package':'zork-ui','gpui':'1.17.0-pre','wasm_bindgen':BINDGEN,'wasm_bytes':wasm.stat().st_size,'compressed_bytes':wasm.with_suffix('.wasm.gz').stat().st_size,'network':'in-memory fixtures only','cjk_font':'Noto Sans SC (OFL); native system fallback remains PingFang SC'},indent=2)+'\n')
