...
with sync_playwright() as pw:
 ...
 choose('field-secret');click('story-field');page.keyboard.press('Control+a');ime=page.context.new_cdp_session(page);value='a密👩‍💻é';end=len(value.encode('utf-16-le'))//2;ime.send('Input.imeSetComposition',{'text':value,'selectionStart':end,'selectionEnd':end});ime.send('Input.insertText',{'text':value});page.wait_for_timeout(150);page.screenshot(path=str(out/'password-unicode.png'));assert state()['text']=='[redacted]'
 ...
 page.keyboard.press('Control+a');page.keyboard.press('Backspace');page.wait_for_timeout(100);page.screenshot(path=str(out/'password-cleared.png'))
