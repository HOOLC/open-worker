...
with sync_playwright() as pw:
 ...
 if not a.reference_only:
  ...
  choose('model-create-compact');click('profile-model-save');assert any(e.get('label')=='填写供应商提供的模型 ID。' and e['visible'] for e in elements());click('profile-model');page.keyboard.type('component-style-model');page.wait_for_timeout(150);assert not any(e.get('label')=='填写供应商提供的模型 ID。' and e['visible'] for e in elements());click('profile-context-limit');page.keyboard.press('Control+a');page.keyboard.type('0');click('profile-model-save');assert any(e.get('label')=='请输入大于 0 的整数。' and e['visible'] for e in elements());click('profile-context-limit');page.keyboard.press('Control+a');page.keyboard.type('32000');click('profile-model-save');assert not has('model-editor-dialog');mark('gpui-model-inline-errors-correct-save')
