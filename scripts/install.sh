...
fetch() {
    curl --fail --silent --show-error --location --retry 2 --connect-timeout 15 \
        --max-time 600 --proto '=https,file' --proto-redir '=https' "$1" -o "$2"
}
version_at_least() {
...
main() {
    ...
    case "$base_url" in https://*|file:///*) ;; *) fail 'Release source must use HTTPS (or file:/// for offline installation)' ;; esac
    base_url=${base_url%/}
