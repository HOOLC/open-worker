#!/usr/bin/env python3
"""Real Gateway/Agent processes, dynamic tools and isolated fake model inputs."""
...
bindings={name:f'127.0.0.1:{f.port()}' for name in ('gateway','runtime','control','agent')}
...
def start():
    ...
    f.wait(lambda:req('GET','/readyz')[0]==200,'gateway ready');f.wait(lambda:req('GET','/readyz',base=agent_url)[0]==200,'agent ready')
def stop():
    global process
    if process:
        process.terminate()
...
try:
    ...
    with sqlite3.connect(root/'state/gateway.sqlite') as db:
    ...
    with sqlite3.connect(root/'state/gateway.sqlite') as db:assert before==db.execute('SELECT session_id FROM worker_tasks ORDER BY request_id').fetchall()
