...
def aggregate_event_records(records: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    for record in records:
        if record.get("kind") != "domain":
            continue
        event = record.get("event")
        if not isinstance(event, dict):
            continue
        event_type = event.get("type")
        if event_type == "activation_started":
            activation_outcome = None
            end_tool_call_ids.clear()
            completion_end_tool_succeeded = False
        elif event_type == "model_attempt_started":
            provider_requests += 1
        ...
        elif event_type == "context_handoff_created":
            context_handoffs += 1
            ...
            final_assistant_tool_call_count = (
                len(message_tool_calls) if isinstance(message_tool_calls, list) else 0
            )
            tool_calls_count = final_assistant_tool_call_count
            total_tool_calls += tool_calls_count
            multi_tool_call_rounds += int(tool_calls_count > 1)
            max_tool_calls_per_round = max(max_tool_calls_per_round, tool_calls_count)
            if isinstance(message_tool_calls, list) and len(message_tool_calls) == 1:
                call = message_tool_calls[0]
                if (
                    isinstance(call, dict)
                    and call.get("tool_name") == "end"
...
def _is_atomic_successful_end(
    ...
    if (
        tool_record is None
        or tool_record.get("batch_index") != 0
        or tool_record.get("batch_size") != 2
        or finish_record.get("batch_index") != 1
        or finish_record.get("batch_size") != 2
    ):
        return False
    event = tool_record.get("event")
    message = event.get("message") if isinstance(event, dict) else None
    return (
        isinstance(event, dict)
        and event.get("type") == "message_appended"
        and isinstance(message, dict)
        and message.get("role") == "tool"
        and message.get("is_error") is False
        and message.get("tool_call_id") in end_tool_call_ids
    )


def iter_session_event_records(session_dir: Path) -> Iterator[dict[str, Any]]:
    segments_dir = session_dir / "segments"
    for segment in sorted(segments_dir.iterdir(), key=lambda path: path.name):
