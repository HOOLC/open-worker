...
from scripts.benchmarks.compare_deepswe import (
    OFFICIAL_CONFIG,
    build_comparison,
    load_zork_trials,
    render_markdown,
...
class DeepSweComparisonTest(unittest.TestCase):
    ...
    def test_matches_pier_display_names_to_official_task_ids(self) -> None:
        zork = [
            zork_result(
                "datacurve/task-a", 1, reward=1, input_tokens=100
            )
        ]
    ...
    def test_reports_errors_and_provider_attempts_without_usage(self) -> None:
        ...
        self.assertEqual(
            report["zork"]["provider_usage"]["attempts_without_usage"], 1
        )
    ...
    def test_reads_provider_request_missing_usage_instead_of_completed_step_gap(
        ...
        self.assertEqual(
            report["zork"]["provider_usage"]["attempts_without_usage"], 2
        )
    ...
    def test_runner_runs_four_attempts_of_exactly_one_named_task(self) -> None:
        runner = (Path(__file__).parent / "run-deepswe.sh").read_text()
        pier_command = runner[runner.index("pier run") :]

        self.assertIn('--include-task-name "$task_name"', pier_command)
        self.assertIn("--n-attempts 4", pier_command)
        self.assertIn("--n-concurrent 4", pier_command)
        self.assertNotIn("--n-tasks", pier_command)
        self.assertNotIn("--sample-seed", pier_command)

    def test_runner_never_treats_an_existing_default_binary_as_current(self) -> None:
        runner = (Path(__file__).parent / "run-deepswe.sh").read_text()

        self.assertIn('if [[ -n "${ZORK_DEEPSWE_BINARY:-}" ]]', runner)
        default_start = runner.index('else\n  binary="$repo_root/target/deepswe/')
        default_end = runner.index("\nfi", default_start)
        default_branch = runner[default_start:default_end]
        self.assertIn(
            '"$script_dir/build-zork-agent-linux-amd64.sh" "$binary"',
            default_branch,
        )
        self.assertNotIn("if [[", default_branch)

    def test_runner_controls_non_streaming_at_agent_startup(self) -> None:
        runner = (Path(__file__).parent / "run-deepswe.sh").read_text()

        self.assertIn("ZORK_DEEPSWE_NO_STREAMING", runner)
        self.assertIn('--agent-kwarg "no_streaming=$no_streaming"', runner)
        self.assertIn('--streaming true', runner)

    def test_cli_requires_an_explicit_expected_task_count(self) -> None:
        comparator = (Path(__file__).parent / "compare_deepswe.py").read_text()

        self.assertIn('parser.add_argument("--expected-task-count"', comparator)
        self.assertNotIn("expected_task_count=10", comparator)
