...
kernel void gpu_lexer_g(uint3 ne [[threadgroup_position_in_grid]], uint3 Ta [[thread_position_in_threadgroup]], uint tint_local_index [[thread_index_in_threadgroup]], const device tint_array<uint, 1>* M [[buffer(0)]], device tint_array<atomic_uint, 1>* Fd [[buffer(1)]], const constant ve* F [[buffer(2)]], const device tint_array<we, 1>* N [[buffer(3)]], const device tint_array<float, 1>* i [[buffer(4)]], device tint_array<half, 1>* ua [[buffer(5)]], const device tint_array<xe, 1>* va [[buffer(6)]], device tint_array<float, 1>* l [[buffer(7)]], threadgroup tint_symbol_4* v_19 [[threadgroup(0)]], const constant tint_immediate_data_struct* tint_immediate_data [[buffer(30)]]) {
