...
def build_native_inventory(shell):
    ...
    for label,source in SOURCES.items():
        ...
        registered=set(re.findall(r'"((?:cue|icons|brand|providers|avatars|illustrations|motion)/[^"\s]+\.svg)"',registry))
        refs={}
        ...
        for file in (file for root in code_roots for file in root.rglob('*.rs')):
            ...
            for line,text in enumerate(file.read_text().splitlines(),1):
                for path in re.findall(r'"((?:cue|icons|brand|providers|avatars|illustrations|motion)/[^"\s]+\.svg)"',text):
        ...
        for p in [source/'assets/cue/README.md',source/'assets/cue/icons.json',source/'assets/icons/PHOSPHOR_LICENSE.txt',source/'assets/providers/sources.json']:
            if p.exists():
    ...
    cards=[]
    for item in items:
        status='当前代码直接引用' if item['references'] else ('来源保留 · 无直接引用证据' if item['family']=='cue' else ('已注册 · 无直接引用证据' if item['registered'] else '目录资源 · 未注册'))
    ...
    body=filters+''.join(cards)+'</div><p class="muted">Cue 图标为项目已授权复用的公司资源，来源见快照中的 README / icons.json；其许可不由 Inter 字体许可替代。Phosphor 保留原 MIT 许可。两个快照中的同名文件可能来自不同设计阶段。</p>'+script
