...
type InputPayloadKind = "structured_message" | "recovered_message_batch" | "background_job_event" | "admin_session_reset";
...
function extractInputPayload(text: string): ExtractedInputPayload | undefined {
  ...
  const namedPayload = extractNamedJsonPayload(text, "recovered_message_batch_json", "recovered_message_batch") ?? extractNamedJsonPayload(text, "background_job_event_json", "background_job_event") ?? extractNamedJsonPayload(text, "admin_session_reset_json", "admin_session_reset");
  if (namedPayload) {
    return namedPayload;
  }
