  ...
  source.addEventListener("admin-event", (message) => {
    try {
      const payload = JSON.parse((message as MessageEvent).data) as {
        readonly event?: AdminRealtimeEvent;
      };
      if (payload.event) {
...
function timelineEventIdentity(event: Record<string, unknown>): string {
  return [event.id, event.at, event.type, event.callId, event.toolName, event.title, event.summary].filter(Boolean).join("\u001f");
}
...
function summarizeSessionCounts(sessions: readonly Record<string, unknown>[]): Record<string, number> {
  ...
  for (const session of sessions) {
    blockedInboundCount += Number(session.blockedInboundCount || 0);
...
const sessionDerivedStateKeys = ["sessionCount", "blockedInboundCount", "backgroundJobCount", "runningBackgroundJobCount", "failedBackgroundJobCount"];
