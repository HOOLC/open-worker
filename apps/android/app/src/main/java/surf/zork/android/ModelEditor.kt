...
internal fun ModelEditor(source: JSONObject?, state: MobileSettingsState, actions: SettingsActions, dismiss: () -> Unit, saved: () -> Unit) {
    ...
    var id by remember { mutableStateOf(initial.text("id")) }
    var api by remember { mutableStateOf(initial.text("api")) }
    var context by remember { mutableStateOf(initial.text("context")) }
    var output by remember { mutableStateOf(initial.text("output")) }
    var levels by remember { mutableStateOf(initial.text("thinking")) }
    var defaultThinking by remember { mutableStateOf(initial.text("default_thinking")) }
    var images by remember { mutableStateOf(initial.optBoolean("images")) }
    var copied by remember { mutableStateOf<JSONObject?>(null) }
    ...
    val templates = remember(state.profiles) {
        state.profiles.flatMap { profile -> profile.optJSONArray("models").objects().filter { NativeBridge.copyableModel(it.toString()) }.map {
    ...
    SettingsSheet(if (source == null) "添加模型" else "编辑模型", busy, error, dismiss, footer = {
        SettingsButton(if (busy) "保存中…" else "保存模型", true, !busy && state.online) {
            attempted = true
            if (errors.isEmpty()) submit {
                val profile = state.profile ?: error("连接已不可用，请返回列表")
                actions.perform("save_model", JSONObject().put("profile", profile.text("profile_id")).put("input", input()))
            }
        }
        if (source != null) SettingsButton("移除模型", enabled = !busy && state.online) { submit {
            val profile = state.profile ?: error("连接已不可用，请返回列表")
            actions.perform("remove_model", JSONObject().put("profile", profile.text("profile_id")).put("model", source))
        } }
    }) {
