// Vendored from rustls-platform-verifier v0.7.0; see ../README.md and LICENSE-MIT.
...
// The upstream library BuildConfig is not generated by this application.
private object BuildConfig { const val TEST = false }
...
internal object CertificateVerifier {
    ...
    private val failures = VerificationFailures<String, VerificationResult>()

    @Synchronized
    internal fun clearFailures() { failures.clear() }
    ...
    private fun verifyCertificateChain(
        ...
        // Cache failures only: a replacement certificate or OCSP staple must be
        // checked immediately, and successful handshakes always validate again.
        val digest = MessageDigest.getInstance("SHA-256")
        fun field(bytes: ByteArray) {
            digest.update(java.nio.ByteBuffer.allocate(4).putInt(bytes.size).array())
            digest.update(bytes)
        }
        field(serverName.toByteArray(Charsets.UTF_8))
        field(authMethod.toByteArray(Charsets.UTF_8))
        field(allowedEkus.joinToString("\u0000").toByteArray(Charsets.UTF_8))
        certChain.forEach(::field)
        field(ocspResponse ?: byteArrayOf())
        val key = digest.digest().joinToString("") { "%02x".format(it) }
        failures.get(key)?.let { return it }
        val result = verifyUncached(context, serverName, authMethod, allowedEkus, ocspResponse, time, certChain)
        if (result.code == StatusCode.Revoked.value || result.code == StatusCode.UnknownCert.value) {
            failures.put(key, result)
        }
        return result
    }

    private fun verifyUncached(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            // checkServerTrusted just established this exact trust anchor.
            // Reuse it for revocation instead of reloading every system CA on
            // every handshake. No trusted outcome survives across handshakes.
            val parameters = PKIXParameters(setOf(TrustAnchor(validChain.last(), null)))
            ...
            revocationChecker.options = revocationOptions(
                ocspResponse != null,
                endEntity.getExtensionValue("1.3.6.1.5.5.7.1.1"),
                endEntity.getExtensionValue("2.5.29.31")
            )
            ...
            try {
                validator.validate(certFactory.generateCertPath(validChain.dropLast(1)), parameters)
    ...
    // Keep explicit stapled OCSP validation and OCSP-capable certificates on
    // the upstream path. CRL-only issuers need CRL checking: Android otherwise
    // treats the absence of an OCSP URL as a hard error, even with SOFT_FAIL.
    internal fun revocationOptions(
        stapledOcsp: Boolean,
        authorityInfoAccess: ByteArray?,
        crlDistributionPoints: ByteArray?
    ): EnumSet<PKIXRevocationChecker.Option> {
        val options = EnumSet.of(PKIXRevocationChecker.Option.SOFT_FAIL,
            PKIXRevocationChecker.Option.ONLY_END_ENTITY)
        // DER encoding of id-ad-ocsp (1.3.6.1.5.5.7.48.1). Looking conservatively
        // for this OID never treats an advertised OCSP service as CRL-only.
        val ocspOid = listOf<Byte>(0x06, 0x08, 0x2b, 0x06, 0x01, 0x05, 0x05, 0x07, 0x30, 0x01)
        val hasOcsp = authorityInfoAccess?.asList()?.windowed(ocspOid.size)?.any { it == ocspOid } == true
        if (!stapledOcsp && crlDistributionPoints != null && !hasOcsp) {
