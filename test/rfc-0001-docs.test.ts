...
describe("RFC 0001 documentation", () => {
  ...
  it("documents the reusable real-smoke evidence bundle", async () => {
    ...
    expect(setup).toContain("run the preflight manually");
    ...
    expect(setup).toContain("live admin status and smoke evidence bundles allowlist `recentBrokerLogs` top-level event tokens plus metadata to RFC-safe scalar fields");
    expect(setup).toContain("malformed broker log lines are reported without echoing their raw text");
    expect(setup).toContain("Rollout metadata recursively redacts unsafe nested metadata string fields");
    expect(setup).toContain("preserving safe posture text such as `FEISHU_APP_SECRET=missing`");
    expect(setup).toContain("pre-rollout broker log snapshot as sanitized evidence");
    expect(setup).toContain("preserving known startup markers by name, and redacting other non-structured lines");
    expect(setup).toContain("Ops tooling summarizes backup/data-root coordinates without full host filesystem paths");
    expect(setup).toContain("summarizes active sessions, pending/inflight inbound messages, and background jobs without raw message bodies, job tokens, or job scripts");
    ...
    expect(implementation).toContain("Rollout tooling reports rollout and preflight directories as repo-relative backup coordinates instead of full host filesystem paths");
    expect(implementation).toContain("pre-rollout log snapshot is sanitized to allowlisted structured event/meta fields or redacted non-structured line summaries instead of raw runtime log text");
    expect(implementation).toContain("live admin status and smoke evidence bundles allowlist `recentBrokerLogs` top-level event tokens plus metadata to RFC-safe scalar fields");
    ...
    expect(implementation).toContain("Ops tooling summarizes backup/data-root coordinates without full host filesystem paths");
    expect(implementation).toContain("and additionally summarizes active sessions, open inbound messages, and background jobs without raw message bodies, job tokens, or job scripts");
    ...
    expect(observability).toContain("Allowlist `recentBrokerLogs` top-level event tokens plus metadata in admin status and smoke evidence bundles to RFC-safe scalar fields");
    expect(observability).toContain("malformed broker log lines are reported without echoing their raw text");
    expect(observability).toContain("Recursively redact unsafe nested string fields from rollout metadata");
    expect(observability).toContain("Write rollout pre-rollout broker logs as sanitized evidence snapshots");
    ...
    expect(observability).toContain("Summarize ops backup/data-root coordinates without full host filesystem paths");
    expect(observability).toContain("ops summaries also cover active sessions, open inbound messages, and background jobs without raw message bodies, job tokens, or job scripts");
  ...
  it("keeps the README aligned with the Slack + Feishu user-facing surface", async () => {
    ...
    expect(readme).toContain("redacts non-structured lines instead of copying raw runtime log text");
