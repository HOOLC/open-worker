...
pub fn recover(
    ...
    if let Some(recovery) = recover_latest(query, session_id, hint, tools)? {
        return Ok(recovery);
    }
...
/// Snapshot plus its append-only suffix; aggregate reads must not fall back to
/// traversing historical segments when this independent origin is unavailable.
pub fn recover_latest(
    ...
    if let Some((state, fold_diagnostics)) = recovered {
        ...
        diagnostics.extend(fold_diagnostics);
        return finish(session_id, state, diagnostics).map(Some);
    }
    ...
    match envelope.event {
        ...
        event => {
            if let Err(error) = state.apply(&event, tools) {
                ...
                state.overview.cursor = Some(envelope.event_id);
            }
        }
    }
}
...
fn restore_snapshot(
    ...
    match migrate_snapshot(state_schema_version, snapshot, tools) {
        ...
        Ok(mut state) => {
            state.overview.cursor = Some(event_id.to_owned());
            Some(state)
        }
        Err(error) => {
