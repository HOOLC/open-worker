...
pub fn install(config: &mut RuntimeConfig) -> Result<PathBuf> {
    fs::create_dir_all(&config.zork_bin_dir).context("create zork bin dir")?;
    let zork_call_src = resolve_named_bin("zork-call", config.zork_call_path.as_deref())?;
    let zork_gh_src = resolve_named_bin("zork-gh", config.zork_gh_path.as_deref())?;
    let zork_call_dest = config.zork_bin_dir.join("zork-call");
    let gh_dest = config.zork_bin_dir.join("gh");
    copy_bin(&zork_call_src, &zork_call_dest)?;
...
fn resolve_named_bin(name: &str, explicit: Option<&Path>) -> Result<PathBuf> {
    ...
    anyhow::bail!("{name} not found; cargo build -p zork-call")
}
