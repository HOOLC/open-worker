...
impl MemoryStore {
    pub fn observed(changed: impl Fn() + Send + Sync + 'static) -> Self {
        Self {
...
impl ProfileStore for MemoryStore {
    ...
    fn upsert_probe(&self, profile_id: &str, account: &Value, rate_limits: &Value) -> Result<()> {
        ...
        let value = (checked_at, account.clone(), rate_limits.clone());
        let changed = {
            let mut probes = self.inner.lock().expect("profile status store");
            if probes.get(profile_id) == Some(&value) {
                false
            } else {
                probes.insert(profile_id.to_owned(), value);
                true
            }
        };
        if changed {
...
/// Serialize read-modify-write operations across processes. Lock a stable
/// sidecar, since the profile itself is replaced by atomic rename.
pub(crate) fn lock_profile(paths: &impl ProfilePaths, profile_id: &str) -> Result<fs::File> {
    validate_profile_id(profile_id)?;
    let directory = paths.profiles_root();
    ...
    let file = options.open(directory.join(format!(".{profile_id}.lock")))?;
    file.lock().context("lock profile")?;
    Ok(file)
}
