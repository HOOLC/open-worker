...
// The system process handle owns its process group and terminates it on drop.
// Keep the controlled process faithful to that ownership contract.
impl Drop for ControlledProcessHandle {
    fn drop(&mut self) {
        let mut state = self
            .state
            .value
        ...
        if state.exit.is_none() {
            state.killed = true;
            state.output.take();
            state.exit = Some(VirtualExit::Status(ProcessStatus { success: false }));
            self.state.changed.notify_waiters();
        }
    }
}
