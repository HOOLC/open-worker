...
async fn elements(
    ...
    authorize(&headers, &state)?;
    advance_test_frame(&state).await?;
...
async fn advance_test_frame(state: &ApiState) -> Result<(), ApiError> {
    // Locked/offscreen test sessions have no native frame callbacks. Deliver one
    // explicit test frame before inspecting UI; native performance runs omit it.
    if std::env::var("ZORK_GUI_TEST_DRIVE_FRAMES").as_deref() == Ok("1") {
        request_driver(state, DriverCommand::TestFrame).await?;
    }
    Ok(())
}
