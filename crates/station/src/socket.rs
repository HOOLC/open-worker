...
pub async fn run_connections(state: AppState, mut shutdown: tokio::sync::watch::Receiver<bool>) {
    ...
    let mut config_changes = state.db.realtime.subscribe();
    let mut config_revision = config_changes.borrow().mesh;
    ...
    loop {
        ...
        tokio::select! {
            ...
            changed = config_changes.changed() => {
                ...
                let next=config_changes.borrow_and_update().mesh;
                if next!=config_revision {
                    config_revision=next;
                    if let Err(error)=state.connections.reload_from_disk().await {warn!(%error,"IM connection config reload failed");}
                }
            }
        }
    }
...
async fn run_connection(
    ...
    loop {
        ...
        match connect_once(&state, &runtime, &mut shutdown, &mut retry).await {
...
async fn connect_once(
    ...
    retry: &mut zork_notify::retry::Retry,
) -> Result<()> {
    let slack = runtime
        .config
        .slack()
        ...
        tokio::select! {
            ...
            message = read.next() => {
                ...
                match message.context("slack websocket read")? {
                    Message::Pong(_) => { awaiting_pong = false; retry.reset(); },
