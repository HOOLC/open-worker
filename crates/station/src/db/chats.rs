...
pub(super) fn initialize(conn: &Connection) -> Result<()> {
    ...
    loop {
        let old = conn
         ...
         WHERE NOT EXISTS(SELECT 1 FROM chat_message_facts f WHERE f.message_id=m.message_id)
...
const CHANNEL_SELECT: &str =
    "SELECT chat_id,session_key,title,epoch,created_at,message_count FROM chat_channels";
...
fn legacy_author(conn: &Connection, message: &VisibleMessageRow) -> Result<Author> {
    let delegated =
        message.message_id.starts_with("assignment-") || message.message_id.starts_with("rework-");
...
fn message(conn: &Connection, id: &str) -> Result<Message> {
    let (chat_id, author, text, mentions, reply_to, created_at): (
        String,
        String,
        String,
        String,
        ...
        .query_row(
            "SELECT f.chat_id,f.author,m.text,f.mentions,f.reply_to,m.created_at
         FROM chat_message_facts f JOIN visible_messages m ON m.message_id=f.message_id
...
pub(super) fn record(
    ...
    let inserted = conn.execute("INSERT OR IGNORE INTO chat_message_facts(message_id,chat_id,author_id,author,reply_to,mentions) VALUES (?1,?2,?3,?4,?5,?6)",
        params![row.message_id,chat_id,author.id,serde_json::to_string(&author)?,reply_to,serde_json::to_string(mentions)?])?;
    if inserted == 0 {
        return Ok(vec![]);
    }
    ...
    conn.execute(
        "UPDATE chat_channels SET message_count=message_count+1 WHERE chat_id=?1",
        [&chat_id],
    )?;
...
impl GatewayDb {
    pub fn chat(&self, id: &str) -> Result<ChatRow> {
        self.conn.lock().expect("db mutex").query_row(&format!("{CHANNEL_SELECT} WHERE chat_id=?1 OR session_key=?1 OR session_key IN(SELECT session_key FROM product_tasks WHERE task_id=?1) OR session_key IN(SELECT key FROM sessions WHERE id=?1)"),[id],map_channel).context("chat_not_found")
    }
        ...
        if changed {
            ...
            if next.subscribed {
                if let Some(StartAt::After { message_id }) = &update.start {
                    let after:i64=tx.query_row("SELECT m.sequence FROM visible_messages m JOIN chat_message_facts f ON f.message_id=m.message_id WHERE f.chat_id=?1 AND f.message_id=?2",params![chat,message_id],|r|r.get(0)).context("invalid_message_cursor")?;
                    let mut after = after;
                    loop {
                        let ids=tx.prepare("SELECT m.sequence,m.message_id FROM visible_messages m JOIN chat_message_facts f ON f.message_id=m.message_id WHERE f.chat_id=?1 AND m.sequence>?2 ORDER BY m.sequence LIMIT 128")?
    ...
    pub fn chat_message(&self, chat: &str, id: &str) -> Result<Message> {
        let message = message(&self.conn.lock().expect("db mutex"), id)?;
    ...
    pub fn chat_messages(
        ...
        self.chat(chat)?;
        let conn = self.conn.lock().expect("db mutex");
        let rows=conn.prepare("SELECT m.sequence,m.message_id FROM visible_messages m JOIN chat_message_facts f ON f.message_id=m.message_id
            WHERE f.chat_id=?1 AND (?2 IS NULL OR m.sequence<?2) AND (?3 IS NULL OR instr(lower(m.text),lower(?3))>0)
            ORDER BY m.sequence DESC LIMIT ?4")?.query_map(params![chat,before,query,limit],|r|Ok((r.get::<_,i64>(0)?,r.get::<_,String>(1)?)))?.collect::<rusqlite::Result<Vec<_>>>()?;
        rows.into_iter()
